...
namespace ec_cpp {
        ...
        while (depart_no < size) {
            ...
            while (j < size) {
                for (size_t i = j; i < (depart_no + j); ++i) {
                    ...
                    const typename TDescriptor::Wide tmp2 = typename TDescriptor::Wide(data[i]) + mask -
                                                            typename TDescriptor::Wide(data[i + depart_no]);
                    const typename TDescriptor::Wide tmp1 =
                            typename TDescriptor::Wide(data[i]) + typename TDescriptor::Wide(data[i + depart_no]);
    ...
    struct PolyEncoder_f2e16 final {
            ...
            static AdditiveFFT initalize(const Tables &tables) {
                ...
                for (size_t m = 0ull; m < (Descriptor::kFieldBits - 1); ++m) {
                    ...
                    for (size_t i = m; i < (Descriptor::kFieldBits - 1); ++i) {
                        ...
                        auto j = (1ull << m) - 1ull;
                        while (j < s) {
                            skews_additive[j + s] = Additive{Descriptor::Elt(skews_additive[j]._0 ^ base[i])};
                            j += step;
                        }
                    }

                    const auto idx = Additive{base[m]}.mul(
                            Additive{Descriptor::Elt(base[m] ^ Descriptor::Elt(1))}.toMultiplier(tables), tables);
                    base[m] = Descriptor::kOneMask - idx.toMultiplier(tables);
                    for (size_t i = (m + 1ull); i < (Descriptor::kFieldBits - 1ull); ++i) {
                        const auto b = (Additive{Descriptor::Elt(base[i] ^ 1)}.toMultiplier(tables) +
                                        Descriptor::Wide(base[m])) % Descriptor::Wide(Descriptor::kOneMask);
                ...
                for (size_t i = 1ull; i < (Descriptor::kFieldBits - 1ull); ++i)
                    base[i] = Descriptor::Elt((Descriptor::Wide(Descriptor::kOneMask) - Descriptor::Wide(base[i]) +
                                               Descriptor::Wide(base[i - 1])) % Descriptor::Wide(Descriptor::kOneMask));
        ...
        Result<std::vector<Additive>> encodeSub(Slice<uint8_t> bytes, size_t n, size_t k) {
            ...
            data.reserve((bytes.size() + 1) / sizeof(Descriptor::Elt) + zero_bytes_to_add / sizeof(Descriptor::Elt));
            ...
            data.insert(data.end(), zero_bytes_to_add / sizeof(Descriptor::Elt), Additive{0ull});
